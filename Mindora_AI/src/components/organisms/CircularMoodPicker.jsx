...
export default function CircularMoodPicker({ value, onChange }) {
  ...
  return (
    <div className="w-full select-none">
      {/* Mobile view: Simple, compact grid */}
      <div className="md:hidden grid grid-cols-3 gap-3 w-full max-w-sm mx-auto px-2 py-1">
        {MOODS.map((m) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex flex-col items-center justify-center py-2.5 px-2 rounded-2xl border transition-all cursor-pointer shadow-soft active:scale-95 outline-none",
                isSelected
                  ? "border-primary font-bold scale-[1.02]"
                  : "border-primary/10 hover:border-primary/30"
              )}
              style={{
                backgroundColor: isSelected ? `${m.color}33` : 'white',
                borderColor: isSelected ? 'var(--color-primary)' : '',
              }}
            >
              {/* 3D Glassy reflection sheen */}
              <div className="absolute top-0.5 left-1 w-[calc(100%-8px)] h-2 rounded-t-full bg-white/20 pointer-events-none" />
              <span className="text-2xl mb-1 filter drop-shadow-sm select-none">{m.emoji}</span>
              <span className="font-ui text-[11px] text-text-main">{m.label}</span>
            </button>
          )
        })}
      </div>

      {/* Desktop view: Circular Mood Picker */}
      <div className="hidden md:flex relative w-80 h-80 mx-auto items-center justify-center">
        ...
        <svg className="absolute w-full h-full pointer-events-none" viewBox="0 0 200 200">
          ...
          {/* Decorative elements */}
          {/* Small brass-gold circle dot */}
          <circle cx="100" cy="30" r="3" fill="#C9A227" className="opacity-80" />

          {/* Gradient Definitions */}
          <defs>
            <linearGradient id="track-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#1E4E79" stopOpacity="0.4" />
              <stop offset="50%" stopColor="#6FA3D0" stopOpacity="0.5" />
            ...
            <button
              ...
              style={{
                ...
                boxShadow: isSelected
                  ? '0 10px 25px -5px rgba(30, 78, 121, 0.4), inset 0 -4px 6px rgba(0, 0, 0, 0.05)'
