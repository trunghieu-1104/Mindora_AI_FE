...
function JournalCard({ entry, onEdit, onDelete }) {
  ...
  return (
    <motion.div
      ...
      className="card border border-[#E8E1CF] bg-white hover:shadow-soft transition-shadow duration-300 relative overflow-hidden"
    >
      {/* Decorative vertical colored mood border */}
      <div 
        className="absolute top-0 left-0 bottom-0 w-1.5" 
        style={{ backgroundColor: mood?.color || '#1E4E79' }}
      />
      ...
      <div className="pl-3">
        ...
        {entry.tags?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {entry.tags.map(t => <Tag key={t} className="bg-bg text-text-sub border border-[#F0EBDC] text-[10px] py-0.5 px-2.5">{t}</Tag>)}
          </div>
        )}
      </div>
    </motion.div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-bg border border-[#E8E1CF] rounded-4xl shadow-soft w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 md:p-8 relative"
      >
        ...
        <div className="mb-5">
          ...
          <div className="relative rounded-2xl overflow-hidden border border-[#E8E1CF] bg-white shadow-inner">
            ...
            <textarea
              ...
              rows={7}
              className="w-full bg-[#FBF7EC] notebook-paper border-0 resize-none font-body text-[#16233D] text-sm leading-8 focus:outline-none focus:ring-0 p-5 placeholder-text-sub/50"
            />
        ...
        <div className="mb-6">
          ...
          <input
            ...
            className="w-full bg-white border border-[#E8E1CF] rounded-xl px-4 py-2.5 font-body text-sm text-text-main placeholder-text-sub/40 focus:outline-none focus:border-primary focus:ring-2 focus:ring-primary/15 transition-all"
          />
        </div>
      ...
      {!loading && (
        <div className="flex flex-col gap-5">
          {allEntries.length > 0 ? (
            ...
            <div className="text-center py-14 card border border-[#E8E1CF] bg-white">
